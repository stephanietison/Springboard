...
SELECT 
	...
	firstname
	FROM `Members`
	WHERE joindate = (SELECT MAX(joindate) AS last_member FROM `Members`);
...
SELECT DISTINCT 
	CONCAT(m.firstname, CONCAT(' ', m.surname)) AS member_name,
...
SELECT
	...
	FROM (
		...
		JOIN `Bookings` AS b ON m.memid = b.memid
		JOIN `Facilities` AS f ON b.facid = f.facid
		WHERE b.starttime LIKE '2012-09-14%') AS cost_query
	WHERE cost > 30
	ORDER BY cost DESC;
...
SELECT
	...
	FROM (
		...
		FROM `Bookings` AS b
		JOIN `Facilities` AS f ON b.facid = f.facid) AS cost_query
